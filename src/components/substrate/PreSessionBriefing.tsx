...
export const PreSessionBriefing = memo(function PreSessionBriefing({
  ...
  if (isLoading) {
    return (
      <div className={cn("bg-growth-teal rounded-lg p-4 text-white", className)}>
  ...
  if (error) {
    return (
      <div
        className={cn("bg-destructive/10 border-destructive/20 rounded-lg border p-4", className)}
      >
        <div className="flex items-center gap-3">
  ...
  return (
    <motion.div
      ...
      className={cn("bg-growth-teal overflow-hidden rounded-lg text-white shadow-md", className)}
    >
      <div className="p-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/80">
                {data.patientName} • {data.appointmentTime}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span>
              {data.lastSessionDate
                ? new Date(data.lastSessionDate).toLocaleDateString()
                : "First session"}
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            ...
            <div className="space-y-4 px-4 pb-4">
              {data.outcomeScores.length > 0 && (
                <div className="rounded-lg bg-white/10 p-3">
                  <h4 className="mb-2 text-sm font-medium">Outcome Measures</h4>
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    {data.outcomeScores.map((score) => {
                      const trend = getTrend(score.currentScore, score.previousScore);
                      const TrendIcon = trend.icon;
                      return (
                        <div key={score.measureType} className="rounded bg-white/10 p-2">
              ...
              {data.riskFactors.length > 0 && (
                <div className="rounded-lg border border-red-400/30 bg-red-500/20 p-3">
              ...
              {data.suggestedTopics.length > 0 && (
                <div className="rounded-lg bg-white/10 p-3">
                  <h4 className="mb-2 text-sm font-medium">Suggested Topics</h4>
