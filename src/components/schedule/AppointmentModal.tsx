...
import { APPOINTMENT_TYPES, type AppointmentTypeCode, type AppointmentTypeConfig } from "@/src/lib/supabase/scheduling-types";
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Popover open={patientOpen} onOpenChange={setPatientOpen}>
              ...
              <PopoverContent className="w-[400px] p-0" align="start">
                <Command>
                  <CommandInput placeholder="Search by name..." value={patientSearch} onValueChange={setPatientSearch} />
                  <CommandList>
                    <CommandEmpty>
                    ...
                    <CommandGroup>
                      {patientsData?.map((patient: Patient) => (
                        <CommandItem key={patient.id} value={`${patient.first_name} ${patient.last_name}`} onSelect={() => handlePatientSelect(patient)} className="min-h-[44px]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input type="number" value={duration} onChange={(e) => setDuration(Number(e.target.value))} min={15} max={180} step={15} className="h-11" />
            </div>
            <div className="space-y-2">
              <Label>Format</Label>
              <div className="flex gap-2">
                <Button type="button" variant={appointmentFormat === "in_person" ? "default" : "outline"} onClick={() => setAppointmentFormat("in_person")} className="flex-1 h-11">
                  <MapPin className="h-4 w-4 mr-2" />
                  In-Person
                </Button>
                <Button type="button" variant={appointmentFormat === "telehealth" ? "default" : "outline"} onClick={() => setAppointmentFormat("telehealth")} className="flex-1 h-11">
          ...
          {appointmentFormat === "in_person" && (
            <div className="space-y-2">
              <Label>Room</Label>
              <Input value={room} onChange={(e) => setRoom(e.target.value)} placeholder="e.g., Room 101" className="h-11" />
            </div>
          )}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {recurringPattern !== "none" && (
                <div className="flex items-center gap-2">
                  <Input type="number" value={recurringOccurrences} onChange={(e) => setRecurringOccurrences(Number(e.target.value))} min={2} max={52} className="h-11 w-20" />
          ...
          <div className="space-y-2">
            <Label>Notes</Label>
            <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Add any notes about this appointment..." rows={3} />
          </div>
