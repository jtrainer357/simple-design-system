...
export function getSuggestedCPTCode(
  ...
  switch (noteType) {
    ...
    default:
      codes = CPT_CODES.filter((c) => ["90832", "90834", "90837"].includes(c.code));
  }
...
export function getApplicableCPTCodes(noteType: NoteType): CPTCode[] {
  switch (noteType) {
    ...
    case "collateral_contact":
      return [];
    default:
      return CPT_CODES.filter((c) => ["90832", "90834", "90837", "90847"].includes(c.code));
  }
}
...
export type OutcomeMeasureType = "PHQ-9" | "GAD-7" | "PCL-5" | "AUDIT" | "DAST-10" | "Columbia-CSS";
 ...
 */
export function calculateTrend(scores: number[]): "improving" | "worsening" | "stable" | null {
