...
interface EnvVarConfig { name: string; required: boolean; description: string; sensitive?: boolean; }
...
export interface AuditResult { valid: boolean; missing: string[]; warnings: string[]; present: string[]; }

export function auditEnvironmentVariables(): AuditResult {
  const missing: string[] = [], warnings: string[] = [], present: string[] = [];
...
export function logAuditResults(result: AuditResult): void {
  ...
  else { console.error("[Env Audit] Missing required environment variables:"); result.missing.forEach((v) => console.error(`  - ${v}`)); }
  if (result.warnings.length > 0) { console.warn("[Env Audit] Warnings:"); result.warnings.forEach((w) => console.warn(`  - ${w}`)); }
}
...
export function validateEnvironment(): void {
  ...
  if (!result.valid && process.env.NODE_ENV === "production") throw new Error(`Missing required environment variables: ${result.missing.join(", ")}`);
}

export function getEnv(name: string, defaultValue?: string): string {
  const value = process.env[name];
  if (!value && defaultValue === undefined) throw new Error(`Environment variable ${name} is not set`);
